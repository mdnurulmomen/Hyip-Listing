...
<body>
    <form action="https://perfectmoney.is/api/step1.asp" method="POST" id="payment_form">
        ...
        <input type="hidden" name="PAYEE_NAME" value="{{$gnl->website_title}}">
        <input type='hidden' name='PAYMENT_ID' value='{{ $perfect['track'] }}'>
        <input type="hidden" name="PAYMENT_AMOUNT"  value="{{$perfect['amount']}}">
        <input type="hidden" name="PAYMENT_UNITS" value="USD">
        <input type="hidden" name="STATUS_URL" value="{{route('ipn.perfect.payment')}}">
        <input type="hidden" name="PAYMENT_URL" value="{{route('user.checkout.success')}}">
        <input type="hidden" name="PAYMENT_URL_METHOD" value="POST">
        <input type="hidden" name="NOPAYMENT_URL" value="{{route('user.gateways')}}">
